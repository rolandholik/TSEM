 ...
 * @magazine_size: The number of struct tsem_coefficient structures that
 ...
 *	slots.
 * @magazine: An array of pointers to struct tsem_coefficient_point
 *	      structures that are pre-allocated for security hooks called
 *	      in atomic context.
 *
 ...
 * maintained by this structure is a list of struct tsem_coefficient
...
struct tsem_model {
	...
	unsigned long *magazine_index;
	struct tsem_work *ws;
	struct tsem_coefficient **magazine;
};
...
/**
 * struct tsem_coefficent - TSEM security coefficient description
 * @list: The list of all the security state coefficients for a
 *	   modeling namespace.
 * @lookup_list: A list referencing a hash table of digest lists for
 *		 the namespace.
 ...
 * struct tsem_coefficient structure will be generated regardless of
 * how many times the security event that generates the coefficient
 * occurs.  The count member of this structure represents the total
 * number of security events that have occurred generated this point.
 *
 ...
 *
 * The lookup of a security coefficient is a hot path, given that
 * for an internally modeled namespace each security event requires
 * the verification of whether or not the coefficient exists.  A
 * second list member in this structure is used to reference a hash
 * table of 256 lists that are selected by the most significant byte
 * of the coefficient.
 *
